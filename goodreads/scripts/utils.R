...
month_plot <- function(df, name, date_col, page_col, title_col,
  ...
  if (save == T){
    ggsave(paste0('Novelty/goodreads/Graphs/', name, '/Monthly_pages_read_', name, '.jpeg'), width=15, height=9, dpi=180)
  }
}
...
year_plot <- function(df, name, fiction_col, date_col, page_col, 
  ...
  if (save == T){
    ggsave(paste0('Novelty/goodreads/Graphs/',  name, '/Yearly_pages_read_', name, '.jpeg'), width=15, height=9)
  }
}
...
read_plot <- function(df, 
  ...
  if (plot){
    ggsave(paste0('Novelty/goodreads/Graphs/', name, '/', plot_name, name, '.jpeg'), width = 16, height=9)
  }
}
...
finish_plot <- function(df, 
  ...
  if (plot == T){
    ggsave(paste0('Novelty/goodreads/Graphs/', name, '/', plot_name, name, '.jpeg'), width=12, height=8)
  }
}
...
year_comparison <- function(l, year_col, year_start, user, plot=T) {
  ...
  if (plot == T){
    ggsave(paste0('Novelty/goodreads/Graphs/', user, '/publication_year_', user, '.jpeg'), width=12, height=8)
  }
}
...
plot_map_data <- function(df, region_dict, world_df, user, country_col = 'nationality_chosen'){
  ...
  ggsave(paste0('Novelty/goodreads/Graphs/', user, '/nationality_map_', user, '.jpeg'), width=12, height=8)
}
...
genre_plot <- function(genre_df, 
  ...
  ggplot(genre_plot_df) + 
    ...
    theme_pander() + theme(plot.title=element_text(hjust=0.5), 
                           legend.position = 'bottom', legend.key.width = unit(1.5, 'cm')) 
  if (plot==T){
    ggsave(paste0('Novelty/goodreads/Graphs/', name, '/', plot_name, name, '.jpeg'), width=14, height=8)
...
summary_plot <- function(dt, date_col, 
  ...
  jpeg(filename = paste0('Novelty/goodreads/Graphs/', name, '/Summary_plot.jpeg'), 
