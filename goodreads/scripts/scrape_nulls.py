...
def reading_percentage():
    ...
    books_df['reading_percentage'] = (books_df['added_by'] - books_df['to_reads']) / books_df['added_by']
    books_df['date'] = books_df['ts_updated'].dt.date
    export_df = objects_to_df(ExportData.objects.all())
    export_df = pd.pivot_table(export_df, index='book_id', values=['title', 'author', 'username'],
