...
import { ActionIcon, CloseButton, Table, TextInput } from "@mantine/core";
import {
  IconCaretDown,
  IconCaretUp,
  IconCaretUpDown,
...
export default function ValueFilterTable<TData, TValue>({
  ...
  return (
    <div className="mx-2 border border-gray-200" style={{ width: tableWidth }}>
      <Table fz="xs">
        <Table.Thead>
          <Table.Tr className="flex w-full">
            {filterTable.getFlatHeaders().map((header) => {
              ...
              const closeButton = (
                <div className={clsx(!searchValue && "hidden")}>
                  <CloseButton
                    iconSize="50%"
